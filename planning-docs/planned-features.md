...
- Add config file to control defaults (done)
...
- Add -z mode, splitting by \0 instead of \n (done)
    - Add option to not print the trailing newline in this mode. Might just add this feature in general (dropped)
        ...
        - RENEW, add an option to control the trailing newline (done via --terminator / -t)
...
- Add feature to skip empty lines (done via --skip-empty-lines / -l)
    - Should it skip empty inputs or empty outputs?
- Add feature to skip lines with no delimiter (done via --skip-undelimited / -s)
...
- Add --terminator / -t to replace the output record terminator (done)
    - Supports string or hex, multichar, and empty (concatenate records)
...
- Build a proper website documenting the current app. (in progress)
    - Make the theme look nice (done)
    - Go through and refine the docs (done)
...
- Zero-indexing (dropped)
- --list to show you a list of each item with its index (dropped)
