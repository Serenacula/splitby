...
# Test 1: Simple split by space
run_test "Simple split by space" "echo 'this is a test' | ./splitby.sh -d '\\s+' 1" "this"
...
run_test "Empty input with error handling" "echo '' | ./splitby.sh -d '\\s+' 1" "Error: No input provided. Use -i/--input or pipe data to stdin."
...
run_test "Empty string with strict bounds" "echo '' | ./splitby.sh -d '\\s+' --strict-bounds 1" "Error: No input provided. Use -i/--input or pipe data to stdin."
...
run_test "Delimiter not provided" "echo 'this is a test' | ./splitby.sh 1" "Error: Delimiter is required. Use -d or --delimiter to set one."

# Test 15: Invalid index format (e.g., '1a' instead of '1')
run_test "Invalid index format" "echo 'this is a test' | ./splitby.sh -d '\\s+' 1a" "Usage: $0 [options] -d <delimiter> index_or_range"

# Test 16: Test with a very large input string
run_test "Test with a very large input string" "echo '$(printf 'a%.0s' {1..1000})' | ./splitby.sh -d '' 500" "a"
